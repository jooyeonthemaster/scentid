...
export const getUserSessionsList = async (limitNum = 50, lastDocumentId = null) => {
  try {
    ...
    // 🔍 해결책: orderBy를 다시 추가하되, JavaScript에서 정렬로 Leading Zero 문제 해결
    let q = query(
      collectionGroup(firestore, 'perfumeSessions'),
      where('status', '==', 'image_analyzed'),
      orderBy('updatedAt', 'desc'),
      limit(limitNum * 2) // 더 많이 가져와서 Leading Zero 사용자들도 포함되도록
    );
    ...
    querySnapshot.forEach((doc) => {
      ...
      // 🔍 임시: 모든 image_analyzed 상태 세션 표시 (디버깅용)
      if (data.status === 'image_analyzed') {
        // 세션 ID에서 타임스탬프 추출 함수
