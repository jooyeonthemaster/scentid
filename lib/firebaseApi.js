...
// 4자리 숫자 기반으로 모든 관련 userId 찾기
export const getUserIdsByPin = async (userPin) => {
  try {
    console.log('🔍 getUserIdsByPin 호출됨:', { userPin });
    ...
    if (snapshot.exists()) {
      ...
      const matchingUserIds = Object.keys(allUsers)
        .filter(userId => userId.startsWith(`${userPin}_`));
      
      console.log(`🔍 4자리 숫자 ${userPin}과 매칭되는 userId: ${matchingUserIds.length}개`);
...
// 4자리 숫자 기반으로 모든 관련 레시피 조회
export const getRecipesByPin = async (userPin) => {
  try {
    console.log('🔍 getRecipesByPin 호출됨:', { userPin });
    
    const matchingUserIds = await getUserIdsByPin(userPin);
    let allRecipes = [];
    ...
    for (const userId of matchingUserIds) {
      try {
        ...
        if (snapshot.exists()) {
          const userRecipes = snapshot.val();
          const formattedRecipes = Object.keys(userRecipes)
            .map(key => ({ 
              id: key, 
              userId: userId,
              ...userRecipes[key],
              createdAt: userRecipes[key].timestamp || userRecipes[key].generatedAt
            }));
          
          allRecipes = allRecipes.concat(formattedRecipes);
        }
      } catch (error) {
        console.error(`사용자 ${userId}의 레시피 조회 실패:`, error);
        // 개별 실패는 무시하고 계속 진행
      }
    }
    ...
    console.log(`🔍 4자리 숫자 ${userPin}의 전체 레시피 ${allRecipes.length}개 조회 완료`);
    return allRecipes;
  } catch (error) {
    console.error('🔍 4자리 숫자 기반 레시피 조회 오류:', error);
    throw error;
  }
};
