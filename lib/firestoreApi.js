...
export const getUserSessionsList = async (limitNum = 50, lastKey = null) => {
  try {
    ...
    // Firestore 컬렉션 그룹 쿼리: updatedAt 최신순, 동률 시 sessionId 역순으로 안정화
    // - orderBy 필드들은 모든 문서에 존재해야 함 (본 프로젝트 세션 문서는 createdAt/updatedAt/sessionId를 항상 보유)
    // - 정확한 커서 페이지네이션(startAfter)을 위해 lastKey에 updatedAt, sessionId를 직렬화/역직렬화하여 사용
    let constraints = [
    ...
    // lastKey 형식: ts:{millis}|sid:{sessionId}
    if (lastKey && typeof lastKey === 'string') {
      try {
        ...
        const sidVal = sidPart.split(':')[1] || '';
        if (!Number.isNaN(tsMillis) && sidVal) {
          // Firestore Timestamp 정렬에 맞춰 Date 객체로 커서 값 전달
          constraints = [
            orderBy('updatedAt', 'desc'),
            orderBy('sessionId', 'desc'),
            startAfter(new Date(tsMillis), sidVal),
            limit(limitNum)
          ];
        }
      } catch (e) {
        console.warn('lastKey 파싱 실패, 첫 페이지로 처리합니다.', lastKey, e);
      }
    }
    ...
    try {
      ...
      // 인덱스 필요 시 임시 폴백: updatedAt 단일 정렬로 동작 유지
      if (err?.message?.includes('requires an index') || err?.code === 'failed-precondition') {
        console.warn('📊 인덱스 없음 - 임시 폴백(updatedAt desc 단일 정렬) 사용');
        let fbConstraints = [orderBy('updatedAt', 'desc'), limit(limitNum)];
        if (lastKey && typeof lastKey === 'string') {
          try {
            const parts = lastKey.split('|');
            const tsPart = parts.find(p => p.startsWith('ts:')) || '';
            ...
            if (!Number.isNaN(tsMillis)) {
              fbConstraints = [orderBy('updatedAt', 'desc'), startAfter(new Date(tsMillis)), limit(limitNum)];
            }
          } catch (e2) {
            console.warn('lastKey 폴백 파싱 실패, 첫 페이지로 처리');
          }
        }
        const fq = query(collectionGroup(firestore, 'perfumeSessions'), ...fbConstraints);
        querySnapshot = await getDocs(fq);
        console.log(`🔥 폴백 쿼리 결과: ${querySnapshot.size}개 문서 조회됨`);
      } else {
        throw err;
      }
    }
    ...
    // 🔧 서버 쿼리에서 이미 최신순 정렬됨. 보정이 필요한 경우에만 최소한의 정렬 유지
    sessions.sort((a, b) => {
    ...
    // 다음 페이지 존재 여부 확인 (limitNum과 동일하게 가져왔을 때만 true)
    const hasMore = querySnapshot.size === limitNum;

    // 커서 직렬화: 쿼리 스냅샷의 마지막 문서 기준(updatedAt, sessionId)
    let nextLastKey = null;
    if (hasMore && querySnapshot.docs.length > 0) {
      const lastDoc = querySnapshot.docs[querySnapshot.docs.length - 1];
      const lastData = lastDoc.data();
      // updatedAt을 밀리초 값으로 변환
      let lastUpdatedAtMs = null;
      try {
        const ts = lastData.updatedAt;
        if (ts && typeof ts.toMillis === 'function') {
          lastUpdatedAtMs = ts.toMillis();
        } else if (ts && typeof ts === 'object' && ts.seconds !== undefined) {
          lastUpdatedAtMs = ts.seconds * 1000 + (ts.nanoseconds || 0) / 1000000;
        } else if (typeof ts === 'number') {
          lastUpdatedAtMs = ts;
        } else if (typeof ts === 'string') {
          const parsed = new Date(ts).getTime();
          lastUpdatedAtMs = Number.isNaN(parsed) ? Date.now() : parsed;
        } else {
      ...
      const lastSessionIdVal = lastData.sessionId || lastDoc.id;
      nextLastKey = `ts:${lastUpdatedAtMs}|sid:${lastSessionIdVal}`;
      console.log('🔍 [lastKey 생성 - cursor]:', nextLastKey);
    }
